...
res3 <- pbsapply(1:ncol(dat_filter),function(i){
  s.init <- shiftprior[i]
  b.init <- theta[i]
  out <- optim(c(softplusinv(b.init), softplusinv(1), s.init, 0, 0.01 * var(Y.shift[,i])),
          nl_matern_advanced, method="L-BFGS-B",
  ...
  Ahat = (ahat^2)*delta+1
  dist = sqrt(plgp::distance(c(t, t2hat)))  # make sure dimensions are appropriate
  dist[dist == 0] <- 1e-8
  
  v = 3/2
  part1 = (bhat * dist)^v
  part2 = (Ahat^(v + 0.5))
  part3 = besselK(bhat * dist, nu = v)
  Vk = (part1 * part3 / part2)%>% as.matrix() + diag(ghat, nrow(Y.shift))  # ensure the diagonal addition
...
write_csv(res3 %>% as.data.frame() %>% `colnames<-`(colnames(dat_filter)),file="data/LIMA_binnedEPCounts_nonstatic.lmatern.estimated.param.csv")

# load("LIMA_binnedEPCounts_nonstatic_res.rda")
# res3<-read.csv(file="data/LIMA_binnedEPCounts_nonstatic.lmatern.estimated.param.csv")
# derive posterior samples' correlation and credible interval
ci <-pbsapply(1: ncol(dat_filter),function(i){
  params <- res3[,i]
  cor <- interpolation.kernel(t,Y_shift=Y.shift[,i],delta,group.size,params,ntest=100, kernel = "lmatern", plot = FALSE)
  return(c(cor[[1]],quantile(cor[[2]],c(0.05,0.95))))
},cl=18)

# plotPair(Y.shift,2,t, s=c(0,shat), mu=muhat)
write_csv(ci %>% as.data.frame() %>% `colnames<-`(colnames(dat_filter)),file="data/LIMA_binnedEPCounts_nonstatic.lmatern.ci.csv")
